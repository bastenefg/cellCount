...
APP = ROOT / "dist/1.2.0/Live-Dead Cell Counter/Live-Dead Cell Counter.exe"
...
def main():
    ...
    required = ["desktop.segmentation", "desktop.segmentation_dialog", "desktop.segmentation_view", "desktop.smoke", "desktop.preview_server", "desktop.quick_start"]
    ...
    run(["--smoke-test", smoke, "--run", ROOT / "runs/gui_existing_sample_validation", "--preview-manifest", ROOT / "samples.csv"], timeout=720)
    ...
    reference = OUT / "packaged_reference_run"
    run(["--pipeline", "reference", "--output", reference, "--log", OUT / "packaged_reference_run.log"], timeout=600)
    validation = json.loads((reference / "reference_validation.json").read_text())
    assert validation["status"] == "passed", validation
    print("Packaged full reference analysis passed.", flush=True)

    # Inspect only windows created by this specific test process; leave all other
    # Live/Dead Cell Counter processes and the user's windows untouched.
    ...
    try:
        ...
        while time.monotonic() < deadline and proc.poll() is None:
            ...
            found = candidates
            if any(title == "Live/Dead Cell Counter" for _, title in found):
                break
            time.sleep(.1)
        matching = [hwnd for hwnd, title in found if title == "Live/Dead Cell Counter"]
        if not matching:
